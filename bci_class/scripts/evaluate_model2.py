"""Like evaluate_model.py but supports the parameterizations."""
import os
import torch
import numpy as np
import pandas as pd
...
# Parameterization stuff if needed for reduced rank.
if args.reduced_rank:
    if args.day_weights_rank:
        svd_compression.apply_parameterization_to_day_weights(model, args.day_weights_rank)
    if args.gru_rank:
        svd_compression.apply_parameterization_to_gru(model, args.gru_rank, args.gru_rank_ih_l0)


# Parameterization stuff if needed for magnitude pruning.
if args.magnitude_pruned:
    magnitude_pruning.apply_parameterization(model)
