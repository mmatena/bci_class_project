...
from bci_class.flops import flops_estimators
...
if args.reduced_rank:
    info = flops_estimators.compute_flops_rank_reduced(
        **common_kwargs,
    ...
    info = flops_estimators.compute_flops_rank_pruned(
        **common_kwargs,
        retain_fraction=args.retain_fraction,
    )

else:
    info = flops_estimators.compute_flops_original_model(**common_kwargs)


print(f'per_patch_steady_state_flops: {info.per_patch_steady_state_flops}')
